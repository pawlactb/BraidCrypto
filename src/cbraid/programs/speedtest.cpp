...
void Encrypt()
{
    ...
    ArtinBraid x(Security_Parameter), y(Security_Parameter), z(Security_Parameter),
        b1(Security_Parameter), b2(Security_Parameter);
    ...
    ArtinBraid::CanonicalFactor f(Security_Parameter / 2);
...
void Decrypt()
{
    ...
    ArtinBraid a1(Security_Parameter), a2(Security_Parameter);
    ArtinBraid c1(Security_Parameter), c2(Security_Parameter), z(Security_Parameter), w(Security_Parameter);
...
int main(int argc, char *argv[])
{
    ...
    OptArg::optmap m;
    m << OptArg::opt("-n", OptArg::int_arg, &Security_Parameter)
      << OptArg::opt("-l", OptArg::int_arg, &CLength)
      << OptArg::opt("-lndex", OptArg::int_arg, &Security_Parameter)
    ...
    // Report parameters.
    cout << "Encryption test, with parameters n=" << Security_Parameter
         << ", l=" << CLength << ", count=" << Count << endl;

    // Compute the block size and security level from Index and CLength.
    double LogFactorial = 0;
    for (int i = 1; i <= Security_Parameter; ++i)
        LogFactorial += log(i);
    LogFactorial /= log(2);
    BlockSize = LogFactorial * 2 * CLength;
    SecurityLevel = CLength * log(Security_Parameter / 2) / log(2);
